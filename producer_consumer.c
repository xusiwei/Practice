...
#define BUFMAX 4
#define CONSOME_LATENCY 1000
#define PRODUCE_LATENCY 500
...
{
	item_t item;
	size_t item_no = (size_t)args;
	// for(;;) {
	int i; for(i=0; i<10; i++) {
		// 1. produce item.
		item = item_no++;
		printf("[producer] I am producing item %d...\n", item);
		msleep(PRODUCE_LATENCY);
...
int main(int argc, char *argv[])
{
	...
	pthread_create(&producer, NULL, producerProc, (void*)1000);
	pthread_create(&consumer, NULL, consumerProc, NULL);
